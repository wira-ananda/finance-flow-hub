...
import { getBusinessUnitName, getUserName } from "@/services/request.service";
...
export function RequestTable({
  ...
  const columns: DataTableColumn<FinanceRequest>[] = [
    {
      ...
      render: (row) => (
        <div className="min-w-0">
          ...
          <p className="mt-0.5 max-w-[280px] truncate text-sm text-foreground">{row.title}</p>
        </div>
      ),
    },
    ...(showUnit
    ...
    },
    {
      ...
      align: "right",
      render: (row) => (
        <span className="num text-sm font-medium text-foreground">{formatRupiah(row.amount)}</span>
      ),
    },
    {
      key: "needed",
      header: "Dibutuhkan",
      render: (row) => (
        <span className="text-sm text-muted-foreground">{formatTanggal(row.neededAt)}</span>
      ),
    },
    {
      key: "status",
  ...
  return (
    <DataTable
      ...
      rows={requests}
      rowKey={(row) => row.id}
      onRowClick={(row) => navigate({ to: "/pengajuan/$id", params: { id: row.id } })}
