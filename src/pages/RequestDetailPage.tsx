...
export function RequestDetailPage({ id }: { id: string }) {
  ...
  const request = useRequest(user, id);
  ...
  return (
    <>
          ...
          <>
            ...
            {availableRequestActions.map((action) => (
              <Button
                ...
                className={
                  ["APPROVE", "SUBMIT", "START_REVIEW", "PROCESS_PAYMENT"].includes(action)
      ...
      <ConfirmationDialog
        ...
        onOpenChange={(open) => {
          if (!open) actions.setPendingAction(null);
        }}
      ...
      <ReviewActionDialog
        open={actions.reviewDialog !== null}
        type={actions.reviewDialog}
        onOpenChange={(open) => {
          if (!open) actions.setReviewDialog(null);
        }}
        onConfirm={actions.handleReviewDecision}
      />

      {actions.paymentDialogOpen && request.status === "APPROVED" ? (
        <PaymentDialog
          key={request.id}
          open
          request={request}
          onOpenChange={actions.setPaymentDialogOpen}
          onConfirm={actions.handlePayment}
        />
      ) : null}
    </>
  );
